...
var _pageService = function() {
    ...
    service.dummyPromise().then(function() {
      ...
      $page.off('submit').on('submit', function(e) {
        if (!e.isDefaultPrevented()) {
          ...
          var loginData = service.getJSON($page.find('form'));
          dataService.login(loginData).then(function(result) {
            if (result.status == true) {
              service.initOverview(this);
            } else {
              $page.find('.alert').removeClass('hide').text("The Password must be the same as the Identity");
            }
          });
  ...
  this.initDetail = function(that) {
    ...
    service.loading($page).then(function() {
      ...
      //binding event
      $page.find('#page_detail_action').off('click').on('click',function() {
